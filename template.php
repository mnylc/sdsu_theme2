 ...
 * Implements hook_form_alter().
 */
function sdsu_theme_form_islandora_collection_search_form_alter(&$form, &$form_state, $form_id) {

  // @author: dpino@metro.org
  // Allow Collection Search to display similar to normal simple one.
  $link = array(
    '#markup' => l(t("Advanced Search"), "advanced-search", array('attributes' => array('class' => array('adv-search')))),
  );
  $form['simple']['advanced_link'] = $link;
  ...
  $form['simple']['collection_select']['#options']['all'] = t('All SDSU collections');
  $form['simple']['islandora_simple_search_query']['#title_display'] = 'invisible';
}
...
function sdsu_theme_preprocess_islandora_solr_wrapper(&$variables) {
  ...
  // @author: dpino@metro.org
  // If query string is empty don't show You searched.
  $querystring = $_islandora_solr_queryclass->solrQuery;
  $variables['your_search'] = "<h1 class='search-main-title'>&nbsp;</h1>";
...
function sdsu_theme_preprocess_page(&$vars) {
  ...
  // dpino@metro.org
  // This search box is being rendered always,
  // if we are using collection search, need to replace by that.
  // @todo This should have been a toggle.
  /* if (!$vars['is_front']) {
    $vars['simple_search'] = sdsu_theme_block_render('islandora_solr', 'simple');

  }*/
   if (!$vars['is_front']) {
     $vars['simple_search'] = sdsu_theme_block_render('islandora_collection_search', 'islandora_collection_search');
  }
